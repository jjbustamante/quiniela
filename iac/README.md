...
- **Custom domain** via Cloudflare DNS + Cloud Run domain mappings: `quiniela.dpdns.org` → web service, `api.quiniela.dpdns.org` → api service. DNS-only mode (no Cloudflare proxy) so Cloud Run can provision its own managed TLS cert. First-time cert provisioning takes ~15-60 min after `tofu apply`.
